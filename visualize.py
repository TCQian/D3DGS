...
import time
from diff_gaussian_rasterization_w_depth import GaussianRasterizer as Renderer
...
def init_camera(y_angle=0., center_dist=4, cam_height=0, f_ratio=0.82):
    ...
    # x-asix rotation matrix
    rx = 90 * np.pi / 180
    w2c = w2c @ np.array([[1., 0., 0., 0.],
...
def load_scene_data():
    ...
    params = dict(np.load("/home/loyot/workspace/code/D3DGS/output/dnerf/hook_fftpoly@20240130-081031/video/ours_None/params.npz"))
    # params = dict(np.load("/home/loyot/workspace/code/D3DGS/output/dnerf/mutant_fftpoly@20240130-085734/video/ours_None/params.npz"))
    # params = dict(np.load("/home/loyot/loyot/workspace/D3DGS/output/dnerf/standup_fftpoly@20240130-091716/video/ours_None/params.npz"))
    # params = dict(np.load("/home/loyot/loyot/workspace/D3DGS/output/dnerf/jumpingjacks_fftpoly@20240130-085352/video/ours_None/params.npz"))
    # params = dict(np.load("/home/loyot/workspace/code/D3DGS/output/hypernerf/split-cookie_fftpoly@20240130-144623/video/ours_None/params.npz"))
    params = {k: torch.tensor(v).cuda().float() for k, v in params.items()}
    is_fg = torch.ones_like(params['means3D'][0][:, 0], dtype=torch.bool, device="cuda")
    is_fg = params['colors_precomp'][0] < 0.9
    is_fg = is_fg.sum(-1) < 2.6
    scene_data = []
    
    for t in range(len(params['means3D'])):
        # tran_mean = params['means3D'][t][:, [0, 2, 1]]
        scales = torch.exp(params['log_scales'])#[:, [0, 2, 1]]
        tran_mean = params['means3D'][t]
        rendervar = {
            'means3D': tran_mean,
            # 'colors_precomp': params['colors_precomp'][t] if not seg_as_col else params['seg_colors'],
            'shs': params['rgb_colors'][t],
...
def calculate_trajectories(scene_data, is_fg):
    ...
    # cols = np.repeat(
    #     colormap[np.arange(len(in_pts[0])) % len(colormap)][None], 
    #     traj_length, 0
    # ).reshape(-1, 3)
    ...
    for t in range(traj_length):
        num_pts = len(in_pts[t])
        alpha = max(1 - 0.9 * ((len(in_pts) - t) / ((len(in_pts) + 1) * .99)), 0.1)
        colors = []
        for j in range(num_pts):
            color_alpha = 1
            
            color = np.array(colo_rmap(j/max(1, float(num_pts - 1)))[:3]) * 255
            hsv = colorsys.rgb_to_hsv(color[0], color[1], color[2])
            color = colorsys.hsv_to_rgb(hsv[0], hsv[1]*color_alpha, hsv[2])
            
            # import ipdb; ipdb.set_trace()
            colors.append(np.asarray(color)[None])
        colors = np.concatenate(colors, 0)
        cols.append(colors[None])
    cols = (np.concatenate(cols, 0) / 255).astype(np.float32).reshape(-1, 3)
...
def visualize(views=None):
    
    scene_data, is_fg = load_scene_data()
    ...
    while RUNING_FLAG:
        ...
        #     if views is None:
            num_loops = 1.4
            y_angle = 360*5*num_loops / num_timesteps
        ...
        if not vis.poll_events():
            RUNING_FLAG=False
        vis.update_renderer()
        # save_image = vis.capture_screen_float_buffer()
        # save_image = Image.fromarray((np.asarray(save_image)*255).astype(np.uint8))
        # save_image.save(f"temps/{frame_id:04d}.png")
        # if frame_id >= 200:
        #     RUNING_FLAG=False
