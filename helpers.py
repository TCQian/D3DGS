...
def setup_camera(w, h, k, w2c, znear=0.01, zfar=100):
    w2c = torch.tensor(w2c).float().unsqueeze(0).transpose(1, 2).cuda()
    # dnerf
    fovx = 0.6911112070083618
    tanHalfFovY = math.tan((fovx / 2))
    ...
    P = P.transpose(0,1).float().cuda()
    
    full_proj = (w2c.bmm(P.unsqueeze(0))).squeeze(0)
